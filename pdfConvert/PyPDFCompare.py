...
class CompareThread(QThread):
    ...
    def __init__(self, files: list[str], progress_window: ProgressWindow = None, options: list[str] = None, parent=None):
    ...
    def mark_differences(self, image1: Image.Image, image2: Image.Image) -> list[Image.Image]:
        ...
        if self.INCLUDE_IMAGES["Overlay"] is True:
            if image1.size != image2.size:
                ...
                print("Comparison", "Page sizes don't match and the 'Scale Pages' setting is off, attempting to match page sizes... results may be inaccurate.")
    ...
    # {files[0]}
    # {files[1]}""")
        print(f"""Processing files: {files[0]}{files[1]}""")
        toc = []
        current_progress = 0
        with fitz.open(files[0 if self.MAIN_PAGE == "NEW" else 1]) as doc1, fitz.open(files[0 if self.MAIN_PAGE == "OLD" else 1]) as doc2:
            ...
            print (f"Total pages {total_operations}.")
            ...
            with TemporaryDirectory() as temp_dir:
                ...
                print ("Compiling PDF from output folder...")
...
-mp:page, --main_page:page  Ex: -mp:NEW
    ...
    Options:
        ...
        OLD
"""
def main(dir, files, options):
    olddir = os.getcwd()
    os.chdir(dir)
    #app = QApplication()
    #progress_window = ProgressWindow()
    #progress_window.show()
    compare_thread = CompareThread(files, None, options)
    compare_thread.run()
    #app.exec()
